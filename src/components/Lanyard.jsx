...
function Band({
  ...
  // Clean solid strap texture with no logo
  const cleanBandTexture = useMemo(() => {
    const canvas = document.createElement('canvas');
    canvas.width = 128;
    canvas.height = 128;
    const ctx = canvas.getContext('2d');
    
    // Clean matte dark strap
    ctx.fillStyle = '#0e1017';
    ctx.fillRect(0, 0, 128, 128);
    
    // Subtle woven edge border
    ctx.fillStyle = '#1c202d';
    ctx.fillRect(0, 0, 10, 128);
    ctx.fillRect(118, 0, 10, 128);

    // Subtle micro-woven grain
    ...
    <>
      <group position={[0, 4, 0]}>
        ...
        <RigidBody position={[2, 0, 0]} ref={card} {...segmentProps} type={dragged ? 'kinematicPosition' : 'dynamic'}>
          <CuboidCollider args={[1.0, 1.45, 0.01]} />
          <group
            scale={2.85}
          ...
          >
            ...
            <mesh geometry={nodes.clip.geometry} material={materials.metal} material-roughness={0.25} />
      ...
      <mesh ref={band}>
        ...
        <meshLineMaterial
          ...
          useMap
          map={lanyardImage ? customTexture : cleanBandTexture}
          repeat={[-4, 1]}
